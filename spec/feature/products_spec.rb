  ...
  describe '関連商品を表示する機能' do
    ...
    context '商品が関連商品を持つ場合' do
      ...
      it '関連商品が順番通りに表示される' do
        # 詳細表示中の商品と同じtaxonを持つ商品が表示される
        # 関連商品はその商品が持つtaxonが属するtaxonomyのposition順（昇順）に表示される。
        # 同じtaxonを持つ商品同士は新しい商品が先に表示される。
        # 該当するtaxonを二つ以上持つ商品はtaxonの属するtaxonomyのpositionの数値が低い商品群に属する
        within('.productsContent') do
          expect(page).to have_selector '.relation-0' , text: product_id3_related_a_b.name
          expect(page).to have_selector '.relation-0' , text: product_id3_related_a_b.display_price.to_s
          expect(page).to have_selector '.relation-1' , text: product_id1_related_a.name
          expect(page).to have_selector '.relation-1' , text: product_id1_related_a.display_price.to_s
          expect(page).to have_selector '.relation-2' , text: product_id2_related_b.name
          expect(page).to have_selector '.relation-2' , text: product_id2_related_b.display_price.to_s
